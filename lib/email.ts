...
// Function to create a fresh transporter (don't reuse)
...
export async function sendClientConfirmation(data: BookingEmailData) {
  const transporter = createTransporter(); // Fresh transporter
  
  try {
    console.log("📧 [EMAIL] Sending client confirmation to:", data.email);
    ...
    console.error("❌ [EMAIL] Failed to send client confirmation:", error.message);
    transporter.close();
    throw error;
  }
}
...
export async function sendOwnerNotification(data: BookingEmailData) {
  ...
  try {
    console.log("📧 [EMAIL] Sending owner notification to:", process.env.OWNER_EMAIL);
